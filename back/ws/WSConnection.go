...
func InitWSConnection(connId uint64, wsSocket *websocket.Conn) (wsConnection *WSConnection) {
	wsConnection = &WSConnection{
		wsSocket:          wsSocket,
		connId:            connId,
		inChan:            make(chan *common.WSMessage, 1000),
		outChan:           make(chan *common.WSMessage, 1000),
...
func (wsConnection *WSConnection) IsAlive() bool {
	...
	// 连接已关闭 或者 太久没有心跳
	if wsConnection.isClosed || now.Sub(wsConnection.lastHeartbeatTime) > time.Duration(60)*time.Second {
		return false
	}
	return true
}
